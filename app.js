(() => {
  ...
  function stripLabel(row){if(row.category==="駅")return assetText(row.categoryName);if(row.category==="信号機")return assetText(row.supplementalB||row.attribute);if(row.category.includes("地上子"))return assetText(row.supplementalB);if(row.category==="曲線")return `${assetText(row.categoryName)}${row.attribute?` R${row.attribute}`:""}`;if(["勾配","‰"].includes(row.category))return row.attribute===null||row.attribute===""?"":`${row.attribute}‰`;if(row.category==="避難口")return assetText(row.categoryName);return row.category==="kp"?`${row.categoryName}k`:assetText(row.categoryName||row.attribute);}
  ...
  function renderAssetStrip(){
    ...
    ctx.setTransform(ratio,0,0,ratio,0,0);ctx.clearRect(0,0,width,height);ctx.fillStyle="#f8f5eb";ctx.fillRect(0,0,width,height);ctx.font="700 10px sans-serif";ctx.fillStyle="#a4322f";ctx.fillText("● 上り",52,14);ctx.fillStyle="#26678e";ctx.fillText("● 下り",96,14);ctx.fillStyle="#59635f";ctx.fillText("◆ 無電源",140,14);ctx.strokeStyle="#d4cfc2";ctx.lineWidth=1;for(let i=0;i<=4;i+=1){const yy=header+i*laneH;ctx.beginPath();ctx.moveTo(0,yy+.5);ctx.lineTo(width,yy+.5);ctx.stroke();}
  ...
  function renderPrintSheet(){
    ...
    $("#printRows").innerHTML=segments.map(([start,end],index)=>`<section class="print-row"><div class="print-row-title"><span>${index+1}　${kmText(start)} ― ${kmText(end)}</span><span>${(end-start).toFixed(3)} km</span></div>${makePrintSvg(start,end,!full)}</section>`).join("");
