(() => {
  ...
  function renderAssetStrip(){
    ...
    ctx.setTransform(ratio,0,0,ratio,0,0);ctx.clearRect(0,0,width,height);ctx.fillStyle="#f8f5eb";ctx.fillRect(0,0,width,height);ctx.font="700 10px sans-serif";ctx.fillStyle="#a4322f";ctx.fillText("● 上り",104,14);ctx.fillStyle="#26678e";ctx.fillText("● 下り",148,14);ctx.fillStyle="#59635f";ctx.fillText("◆ 無電源",192,14);ctx.strokeStyle="#d4cfc2";ctx.lineWidth=1;for(let i=0;i<=4;i+=1){const yy=header+i*laneH;ctx.beginPath();ctx.moveTo(0,yy+.5);ctx.lineTo(width,yy+.5);ctx.stroke();}
  ...
  $("#assetStripToggle").addEventListener("click",event=>{event.stopPropagation();setAssetStripOpen(!state.assetStripOpen);});
